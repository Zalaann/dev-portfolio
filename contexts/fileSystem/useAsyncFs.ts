...
// BrowserFS package's internal paths may not be exported; obtain Stats and FileType via BFSRequire("fs")
type BfsStats = any;
const FsInternal: any =
  BFSRequire && typeof BFSRequire === "function"
    ? (BFSRequire("fs") as any)
    : undefined;
const BFSStatsCtor: new (...args: any[]) => BfsStats = FsInternal?.Stats;
const BFSFileType: any = FsInternal?.FileType ?? { FILE: 0 };
    ...
    () => ({
      ...
      lstat: (path) =>
        new Promise((resolve, reject) => {
          fs?.lstat(path, (error, stats = Object.create(null) as any) =>
      ...
      stat: (path) =>
        new Promise((resolve, reject) => {
          fs?.stat(path, (error, stats = Object.create(null) as any) => {
            if (error) {
              if (UNKNOWN_STATE_CODES.has(error.code)) {
                if (BFSStatsCtor) {
                  return resolve(new BFSStatsCtor(BFSFileType.FILE, -1));
                }
